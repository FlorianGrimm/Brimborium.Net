...
namespace Brimborium.CodeFlow.FlowSynchronization {
    public class SyncDictionaryTests {
        ...
        public async Task SyncDictionary_ExclusiveAsync() {
            ...
            var v1 = (await t1).GetSyncById().GetItem().ToString();
            Assert.Equal("1-1", v1);
        ...
        public async Task SyncDictionary_SharedAsync() {
            ...
            Assert.Equal(TaskStatus.RanToCompletion, t3.Status);
            var d1 = await t1;
            Assert.Equal("1-1", d1.GetSyncById().GetItem().ToString());
            (await t1).Dispose();
            (await t2).Dispose();
            (await t3).Dispose();
        }
        ...
        public async Task SyncDictionary_SharedExclusiveSharedAsync() {
            ...
            Assert.Equal("1-1", d1.GetSyncById().GetItem().ToString());
            var d2 = await t2;
            Assert.Equal("1-1", d2.GetSyncById().GetItem().ToString());
            d1.Dispose();
            d2.Dispose();
            ...
            Assert.Equal(TaskStatus.WaitingForActivation, t4.Status);
            var d3 = await t3;
            Assert.Equal("1-1", d3.GetSyncById().GetItem().ToString());
            d3.Dispose();
            ...
            var d4 = await t4;
            Assert.Equal("1-1", d4.GetSyncById().GetItem().ToString());
            d4.Dispose();
        ...
        public async Task SyncDictionary_SharedDifferentKey() {
            ...
            var d1 = await t1;
            Assert.Equal("one-1", d1.GetSyncById().GetItem().ToString());
            var d2 = await t2;
            Assert.Equal("two-2", d2.GetSyncById().GetItem().ToString());
        }
