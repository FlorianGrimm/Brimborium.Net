...
namespace Brimborium.WebDavServer.FileSystem {
    ...
    public class PathTraversalEngine : IPathTraversalEngine {
        ...
        public Task<SelectionResult> TraverseAsync(IFileSystem fileSystem, string? path, CancellationToken ct) {
            return this.TraverseAsync(fileSystem, SplitPath(path ?? string.Empty), ct);
        }

        /// <inheritdoc />
        public Task<SelectionResult> TraverseAsync(ICollection currentCollection, string? path, CancellationToken ct) {
            return this.TraverseAsync(currentCollection, SplitPath(path ?? string.Empty), ct);
        }
        ...
        private Task<SelectionResult> TraverseAsync(ICollection currentCollection, IEnumerable<string> pathParts, CancellationToken ct) {
            return this.TraverseAsync(currentCollection, ToPathElements(pathParts), ct);
        }
        ...
        private async Task<SelectionResult> TraverseAsync(IFileSystem fileSystem, IEnumerable<string> pathParts, CancellationToken ct) {
            ...
            return await this.TraverseAsync(current, pathParts, ct).ConfigureAwait(false);
        }
        ...
        private async Task<SelectionResult> TraverseAsync(ICollection startCollection, IEnumerable<PathElement> pathParts, CancellationToken ct) {
            ...
            for (var i = 0; i != pathPartsArr.Length; ++i) {
                ...
                if (this._logger?.IsEnabled(LogLevel.Trace) ?? false) {
                    this._logger.LogTrace($"Processing path ({id}), part {pathPart.Name} ({pathPart.OriginalName})");
                }
                ...
                if (next == null) {
                    ...
                    if (this._logger?.IsEnabled(LogLevel.Debug) ?? false) {
                        this._logger.LogDebug($"Processing path ({id}), missing {pathPart.Name} ({pathPart.OriginalName}) with ({string.Join("/", missingPathParts)}) following");
                    }
                ...
                if (pathPart.IsDirectory && !isDirectory) {
                    ...
                    var missingPathParts = pathPartsArr.Select(x => x.Name).Skip(i).ToArray();
                    if (this._logger?.IsEnabled(LogLevel.Debug) ?? false) {
                        this._logger.LogDebug($"Processing path ({id}), missing collection {pathPart.Name} ({pathPart.OriginalName}) with ({string.Join("/", missingPathParts)}) following");
                    }
                ...
                if (!isDirectory) {
                    // file found
                    if (this._logger?.IsEnabled(LogLevel.Debug) ?? false) {
                        this._logger.LogDebug($"Processing path ({id}), found document {next.Name} ({next.Path})");
                    }
            ...
            // directory found
            if (this._logger?.IsEnabled(LogLevel.Debug) ?? false) {
                this._logger.LogDebug($"Processing path ({id}), found collection {currentCollection.Name} ({currentCollection.Path})");
            }
