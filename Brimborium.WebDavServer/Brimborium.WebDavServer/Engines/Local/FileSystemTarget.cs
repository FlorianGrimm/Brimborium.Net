...
namespace Brimborium.WebDavServer.Engines.Local
{
    ...
    public class FileSystemTarget : ITarget
    {
        ...
        public CollectionTarget NewCollectionTarget()
        {
            ...
            Uri collUrl = this.DestinationUrl.GetParent();
            var collTarget = this.Parent == null ? null : CollectionTarget.NewInstance(collUrl, this.Parent, this._targetActions);
            return new CollectionTarget(this.DestinationUrl, collTarget, this.Collection, false, this._targetActions);
        }
        ...
        {
            if (this.Document == null || this.Parent == null) {
                throw new InvalidOperationException();
            }

            Uri collUrl = this.DestinationUrl.GetParent();
            var collTarget = CollectionTarget.NewInstance(collUrl, this.Parent, this._targetActions);
            return new DocumentTarget(collTarget, this.DestinationUrl, this.Document, this._targetActions);
        }
        ...
        public MissingTarget NewMissingTarget()
        {
            ...
            Uri collUrl = this.DestinationUrl.GetParent();
            var collTarget = CollectionTarget.NewInstance(collUrl, this.Parent, this._targetActions);
            return new MissingTarget(this.DestinationUrl, this.Name, collTarget, this._targetActions);
        }
    }
}
