...
namespace Brimborium.CodeGeneration.CodeAnalysis {
    internal static class TypeExtensions {
        public static string GetCompilableName(this Type type) {
            if (type.IsArray) {
                return (
                        type.GetElementType() ?? throw new NullReferenceException("type.GetElementType()")
                    ).GetCompilableName() + "[]";
            }
            ...
            if (!type.IsGenericType) {
                compilableName = type.FullName ?? throw new NullReferenceException("type.FullName ");
            } else {
                StringBuilder sb = new();

                var fullName = type.FullName ?? throw new NullReferenceException("type.FullName ");
                ...
                var genericArgCount = genericArgs.Length;
                List<string> genericArgNames = new(genericArgCount);

                for (var i = 0; i < genericArgCount; i++) {
                    genericArgNames.Add(genericArgs[i].GetCompilableName());
                }
        ...
        public static string GetTypeInfoPropertyName(this Type type) {
            if (type.IsArray) {
                return (
                        type.GetElementType() ?? throw new InvalidOperationException("type.GetElementType()")
                    ).GetTypeInfoPropertyName() + "Array";
            ...
            var name = ((TypeWrapper)type).SimpleName;

            sb.Append(name);

            foreach (var genericArg in type.GetGenericArguments()) {
                sb.Append(genericArg.GetTypeInfoPropertyName());
            }
        ...
        public static bool CanContainNullableReferenceTypeAnnotations(this Type type) {
            ...
            if (type.IsGenericType) {
                foreach (var genericParam in type.GetGenericArguments()) {
                    if (genericParam.CanContainNullableReferenceTypeAnnotations()) {
                        return true;
                    }
                }
            }
