...
namespace Brimborium.TypedStoredProcedure {
    public partial class Generator {
        public static bool GenerateSqlAccessWrapper(
            ...
            var dctTypeNames = types.Where(t => regex.IsMatch(t.Name)).ToDictionary(t => t.Name, t => t.FullName, StringComparer.OrdinalIgnoreCase);
            ...
            {
                var dctSPIsConfigured = lstAllStoredProcedure.Select(sp => sp.SPDefinition).ToDictionary(sp => sp.SqlName, sp => false);
                foreach (var spDef in dbDef.StoredProcedures) {
                    if (dctSPIsConfigured.ContainsKey(spDef.SqlName)) {
                        ...
                        sbOutputImplementation.AppendLine($"#warning {spDef.SqlName} is defined, cannot find in database.");
                    }
                }
            }

            {
                var dctSPIsConfigured = dbDef.StoredProcedures.GroupBy(sp => sp.SqlName, sp => sp).ToDictionary(sp => sp.Key, items => items.ToList());
                foreach (var dbSP in lstAllStoredProcedure) {
                    if (dctSPIsConfigured.TryGetValue(dbSP.SPDefinition.SqlName, out var spDefs)) {
                        ...
                        var argsCSTypeDefinition = string.IsNullOrEmpty(argsTypeFullName)
                            ...
                            ;
                        sbOutputImplementation.AppendLine($"#warning new StoredProcedureDefintion(\"{dbSP.SPDefinition.Schema}\", \"{dbSP.SPDefinition.Name}\", {argsCSTypeDefinition}, ExecutionMode.Unknown, {returnCSTypeDefinition}),");
                    }
                }
            }
        ...
        private static void printFileImplementation(
            ...
            printCurly($"namespace {printClass.Namespace}", "", ctxt, (ctxt) => {
                //ctxt.AppendLine($"public partial class {printClass.ClassName}");

                printCurly($"public partial class {printClass.ClassName}", "", ctxt, (ctxt) => {
                    ...
                    foreach (var (dbSP, spDef) in lstUsed) {
                        if (dbSP is object && spDef is object) {
                            printExecuteMethod(dbSP, spDef, hsReaderDefinition, lstReaderDefinition, storedProcedureNames,
                ignoreTypePropertyNames, ctxt); ;
                        }
                    }
        ...
        private static void printExecuteMethod(
            ...
            //
            var csMethod = $"public {csAsyncModifier}{csCompleteReturnType} {methodName}({csArgs}IDbTransaction? tx = null) ";
            printCurly(csMethod, Environment.NewLine, ctxt, (ctxt) => {
                //
                printCurly($"using(var cmd = this.CreateCommand(\"{dbSP.SPDefinition.SqlName}\", CommandType.StoredProcedure, tx))", "", ctxt, (ctxt) => {
                    ...
                    if (spDef_Argument is object && argument_Type is object) {
                        #region Parameters
                        var dctProperties = argument_Type.GetProperties().ToDictionary(p => p.Name, StringComparer.OrdinalIgnoreCase);
                        foreach (var dbSPParameter in dbSP.Parameters) {
                            ...
                            if (dctProperties.TryGetValue(nameNoAt, out var property)) {
                                ...
                                ctxt.AppendLine($"#warning StoredProcedure defines parameter {parameterName} no matching property found.");
                                continue;
                            }
                            ...
                            if (sqlDataType == SqlDataType.UserDefinedTableType) {
                                ...
                                if (propertyTypeNotNull != dbSPParameter.Type.Type) {
                                    if (propertyTypeNotNull.IsEnum
                                        ...
                                        ctxt.AppendLine($"#warning PropertyType parameter.Type.{property.PropertyType.FullName} {dbSPParameter.Type.Type.FullName}");
                                    }
                                }
                            }
                                ...
                                switch (sqlDataType) {
                                    ...
                                    case SqlDataType.UserDefinedTableType:
                                        if (spDef_Argument_Member is null || spDef_Argument_Member.Type.ParameterConverter is null) {
                                            ...
                                            ctxt.AppendLine($"#warning this.AddParameterStructured(cmd, \"{parameterName}\", args.{propertyName}, ParameterConverter.ConvertParameter);");
                                        } else {
                        ...
                        ctxt.Output.AppendLine("#warning C#-Parameters is none but SQL-Parameters are defined.");
                    }
            ...
            switch (executionMode) {
                ...
                case ExecutionMode.QuerySingle: {
                        if (dbSP_ResultSets.Length != 1) {
                            ctxt.Output.AppendLine($"#warning #ResultSets {dbSP_ResultSets.Length} actual, expected one!");
                            foreach (var dbResultSet in dbSP.ResultSetsToStrings()) {
                                ctxt.Output.AppendLine($"#warning Database: {dbResultSet}!");
                            }
                            ctxt.AppendLine("throw new NotImplementedException();");
                        } else {
                            var readerName = $"ReadRecord{dbSP.SP.Name}";
                ...
                case ExecutionMode.QueryMultiple:
                    ...
                    if (spDef_Return.Members is null || spDef_Return.Members.Length == 0) {
                        ctxt.Output.AppendLine($"#warning No Members defined! {dbSP.ResultSets.Length} found in database!");
                        foreach (var dbResultSet in dbSP.ResultSetsToStrings()) {
                            ctxt.Output.AppendLine($"#warning Database: {dbResultSet}!");
                        }
                        ctxt.AppendLine("throw new NotImplementedException();");
                        ...
                        ctxt.Output.AppendLine($"#warning Different count of resultset({dbSP_ResultSets.Length}) and members({spDef_Return.Members.Length}) defined!");
                        foreach (var dbResultSet in dbSP.ResultSetsToStrings()) {
                            ctxt.Output.AppendLine($"#warning Database: {dbResultSet}!");
                        }
                        ctxt.AppendLine("throw new NotImplementedException();");
            ...
            printCurly($"protected {csReturnTypeRecord} {args.ReaderName}(Microsoft.Data.SqlClient.SqlDataReader reader)", Environment.NewLine, ctxt, (ctxt) => {
                ...
                }
                {
                    foreach (var mappingResult in mappingResults.AllMappings.Where(m => m.Mode == -1)) {
                        ctxt.AppendLine($"#warning Column not mapped {mappingResult.Index}: {mappingResult.Identity} {mappingResult.Column.TypeName} ({mappingResult.Column.TypeSize})");
                    }
                }
                {
                    foreach (var mappingResult in mappingResults.AllMappings.Where(m => m.Mode > 0)) {
                        if (!SQLUtility.DoesTypesMatch(typeName: mappingResult.Column.TypeName, mappingResult.ValueType)) {
                            ...
                            if (rc.returnType.NotNullableType != rc.valueType.NotNullableType) {
                                ctxt.AppendLine($"#warning Column types not match {mappingResult.Column.Name} \"{mappingResult.Column.TypeName}\", typeof({mappingResult.ValueType.FullName})");
                                ctxt.AppendLine($"#warning {rc.returnType.NotNullableType.FullName} != {rc.valueType.NotNullableType.FullName} ");
                            }
#if false
        ...
        private static void printMultiReader(
            ...
            if (members is null) {
                ...
                foreach (var member in members) {
                    ...
                    ctxt.AppendLine($"#warning not found {member.Name}");
                }
                ...
                if (nonmappingResults.Count > 0) {
                    foreach (var c in nonmappingResults) {
                        ctxt.AppendLine($"#warning Property not mached: {c.CsName}");
                    }
                }
                {
                    foreach (var mappingResult in mappingResults.AllMappings.Where(m => m.Mode == -1)) {
                        ctxt.AppendLine($"#warning not mapped {mappingResult.Index}: {mappingResult.Identity} {mappingResult.Column.TypeName} ({mappingResult.Column.TypeSize})");
                    }
                }
#endif
#if true
